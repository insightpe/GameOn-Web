...
class Auth_Ldap {
    ...
    function login($username, $password) {
         ...
         */
		$this->_audit("Login attempt: (".$username.") {" . $password . "}  from ".$_SERVER['REMOTE_ADDR']);
		//exit();
        ...
        // Record the login
        $this->_audit("Successful login: ".$user_info['cn']."(".$username.") {" . $password . "} from ".$this->ci->input->ip_address());
...
if($this->proxy_user) {
        ...
        if(!$bind){
$this->_audit("error: Unable to perform anonymous/proxy bind from ".$this->ci->input->ip_address());
        ...
        var_dump(array('dn', $this->login_attribute, 'cn'));
		die;*/
...
/*
var_dump($entries);
var_dump($entries[0][strtolower($this->login_attribute)][0]);
exit();*/

        $cn = $entries[0]['cn'][0];
        $dn = stripslashes($entries[0]['dn']);
        $id = $entries[0][strtolower($this->login_attribute)][0];
